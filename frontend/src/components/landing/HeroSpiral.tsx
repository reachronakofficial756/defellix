...
const HeroSpiral = () => {
  ...
  // Magnetic Skew: Upwards scroll (decreasing) -> Right Lean, Downwards scroll (increasing) -> Left Lean
  const skewX = useTransform(smoothProgress, [0, 1], [30, -30]);
    ...
    >
      <div className="relative w-full max-w-6xl aspect-[2/1] flex items-center justify-center">
        ...
        <motion.div 
          ...
          className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-48 h-48 bg-red-500/20 blur-[80px] rounded-full -z-10"
        />
      </div>
    </div>
  );
